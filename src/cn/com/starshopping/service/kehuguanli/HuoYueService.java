 ...
 * User: huxiaoyi
 * Date: 2019/01/10
 * Description:  查看活跃用户service层
 * Version: V1.0
 */
public class HuoYueService {

    public JSONObject huoyue(HttpServletRequest request)throws IOException {
        //记录操作
        String kefu = (String)request.getSession().getAttribute(SessionKey.KEFU);
        CaoZuoJiLuDao caoZuoJiLuDao = new CaoZuoJiLuDao();
        caoZuoJiLuDao.chaRuCaoZuoJiLu(kefu, "查看活跃用户");
        //得到前台传入的搜索条件
        ...
        keHuDengLu.setZhuCeShiJian(Timestamp.valueOf(sousuojsonObject.getString("zhucedatemin")));
        ...
        String dengludatemin ="";
        String dengludatemax = sousuojsonObject.getString("dengludatemax");
        if(action.equals("zuijin")){
            int day = Integer.valueOf(sousuojsonObject.getString("day"));
            dengludatemin = (new Timestamp(time-1000*60*60*24*day)).toString().substring(0,10);
        }
        //查看当天活跃用户
        else if(action.equals("day")){
            dengludatemin = nowtime;
        }
        //查看本周活跃用户
            ...
            dengludatemin = new SimpleDateFormat("yyyy-MM-dd").format(cal.getTime());
        }
        //查看本月活跃用户
        else if(action.equals("month")){
            //获取本月第一天的日期
            dengludatemin = nowtime.substring(0,8)+"01";
        }
        //查看本年活跃用户
        else if(action.equals("year")){
            //获取本年第一天的日期
            dengludatemin = nowtime.substring(0,5)+"01-01";
        }
            ...
            dengludatemax = new SimpleDateFormat("yyyy-MM-dd").format(cal1.getTime());
            ...
            dengludatemin = "" + year + "-" + month + "-01";
            dengludatemax = "" + year + "-" + month + "-" + maxday;
        }
        else if(action.equals("syear")){
            int year = Integer.valueOf(nowtime.substring(0,4));
            dengludatemin = (year - 1) + "-01-01";
            dengludatemax = (year - 1) + "-12-31";
        }
        keHuDengLu.setZuiHouYiCiDengLuShiJian(Timestamp.valueOf(dengludatemin));
        jsonObject1 = keHuDao.souSuo(keHuDengLu, dengludatemax,
                sousuojsonObject.getString("zhucedatemax"), page, limit);
        return jsonObject1;
    }
}
